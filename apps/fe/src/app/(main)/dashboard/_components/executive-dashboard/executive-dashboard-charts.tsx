    ...
    <div className="space-y-3">
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => {
          ...
          const color = dashboardStatusColor(item.key);
          return (
            <button
              key={item.key}
              type="button"
              disabled={!onSelect || !isSelectable(item.key)}
              onClick={() => onSelect?.(item.key)}
              aria-pressed={selectedKey === item.key}
              className="block w-full space-y-1.5 rounded-md p-1.5 text-left transition-colors enabled:hover:bg-muted/40 enabled:focus-visible:outline-none enabled:focus-visible:ring-2 enabled:focus-visible:ring-ring disabled:cursor-default aria-pressed:bg-[var(--dc-primary-soft)]"
            >
...
export function CompositionPanel({
  ...
  const panels: Array<{
    ...
    { title: "Sumber Lokasi", items: analytics.locationSource, queryKey: "coordinateSource" },
  ];
