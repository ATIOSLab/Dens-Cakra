...
import { DirectiveDetailClient, DirectiveFormClient, DirectiveListClient, DirectiveTrackingClient } from "./client";
import type {
...
function normalizeRegionalAssignments(
  assignments: PositionAssignmentResource[],
): RegionalAssignmentOption[] {
    ...
    return [
      {
        ...
        positionTitle: assignment.position.title ?? "Regional Commander",
...
export async function DirectiveStrategicListPage() {
...
export function DirectiveAliasListPage() {
  redirect("/dashboard/executive/pusat-komando/direktif-strategis");
}

export function DirectiveStrategicEditAliasPage({ directiveId }: { directiveId: string }) {
  redirect(`/dashboard/executive/pusat-komando/direktif/${directiveId}/edit`);
}
