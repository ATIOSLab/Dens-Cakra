...
function reportCategoryRank(category: ReportCategoryOrderItem) {
  const candidates = [category.code, category.name]
...
export function compareReportCategories<T extends ReportCategoryOrderItem>(
  left: T,
  right: T,
) {
  ...
  const nameDiff = leftName.localeCompare(rightName, 'id', {
    sensitivity: 'base',
  });
...
export function sortReportCategories<T extends ReportCategoryOrderItem>(
