...
| 2026-09-12 | Otoritas Penangguhan Jaring (Suspend/Unsuspend): Role Deputi II (`executive`) memiliki wewenang eksklusif untuk menangguhkan Jaring (`POST /api/v1/jaring/:id/suspend`) dan memulihkan (`POST /api/v1/jaring/:id/unsuspend`). Nilai `SUSPENDED` pada enum `JaringRegistrationStatus` mengecualikan Jaring dari seluruh metrik terverifikasi (KPI produktivitas, summary cards, peta, dan WhatsApp intake). Dicatat di `AuditLog` (`JARING.SUSPEND` & `JARING.UNSUSPEND`). | Kebutuhan pimpinan pusat (Deputi II) untuk menonaktifkan dan mendiskualifikasi Jaring bermasalah dari status terverifikasi. |
| 2026-09-16 | Fitur Apel & Absensi WhatsApp Bot Jaring: Blasting massal anti-ban (jitter delay 8-20s, chunking 5 pesan + cooldown 30s, Baileys presence composing, dynamic spintax & zero-width entropy), konfigurasi multi-bot rotasi per wilayah di Superadmin (`/dashboard/admin-system/pengaturan-apel`), dan visualisasi spasial peta kehadiran Deputi II (`/dashboard/deputi/peta-apel`). Auto-capture kehadiran inbound via Baileys interceptor. | Kebutuhan otomatisasi absensi berkala jaring terverifikasi dengan mitigasi pemblokiran akun WhatsApp dan pemantauan geografis kepatuhan apel. |
