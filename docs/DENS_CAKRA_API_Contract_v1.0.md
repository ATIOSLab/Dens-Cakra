...
| API-USR-002 | `POST /user-profiles/provision` | Provision akun, profile, role dan scope secara atomik | `user.provision` | {"auth":{"name":"...","email":"...","password":"...","role":"executive"},"profile":{"username":"...","fullName":"...","phone":"..."},"assignment":{"branch":"PUSAT","validFrom":"ISO"},"areaScopeIds":["uuid-country-IDN"]} | `201` 201 UserProfileDetail | Transaction: create Better Auth user→UserProfile PENDING→validate role-branch-area→create assignment→create scopes→set ACTIVE→audit. | Email/username unique; `executive` wajib `PUSAT` + scope `COUNTRY`; role operasional memakai `BINDA`/`DIRECTORATE`; tidak ada public self-registration; Idempotency-Key wajib. |
